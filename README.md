...
- **🌐 英中实时翻译**：选择“英文翻译为中文”后，将英文字幕按上下文流式翻译成简体中文译文。
...
AI 设置页支持 DeepSeek API Key、模型、自动判断/总结/问答/解释、中文翻译为英文、英文翻译为中文、停顿触发时间和附加提示词。回答或译文通过 SSE 流式返回并逐字更新 Overlay。API Key 使用 Windows DPAPI 按当前用户加密保存在 `%LOCALAPPDATA%\WasapiParaformerOverlay\deepseek.key`，不会写进 JSON、源码或运行日志。未设置 Key 时不会发送任何 API 请求。

字幕采用左对齐连续视图：每段 `final` 直接显示原文，不再添加“语音”标签；DeepSeek 回答标为“AI”，翻译结果标为“译文”。SSE delta 在后台读取并立即进入打字机缓冲，Overlay 每 35 ms 只显示 1–2 个完整字符。后续内容进入队列，最近多轮原文与 AI 回复或译文会作为上下文继续发送。显示范围不足时自动滚动到最新消息。
